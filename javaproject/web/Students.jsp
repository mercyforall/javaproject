<%-- 
    Document   : Students
    Created on : May 8, 2017, 9:43:51 AM
    Author     : Freeware Sys
--%>
...
<%
        ...
        if(request.getParameter("b1")!=null)
        {
            ...
            String[] option1=new String[12];
            String[] option2=new String[12];
            String[] option3=new String[12];
            String[] option4=new String[12];
            int[] answer=new int[12];
            int[] uans=new int[12];
            int index=-1;
            String scode=request.getParameter("s1");
            ...
            while(rs.next())
            {
                question[i]=rs.getString(3);
                option1[i]=rs.getString(4);
                option2[i]=rs.getString(5);
                option3[i]=rs.getString(6);
                option4[i]=rs.getString(7);
                answer[i]=rs.getInt(8);
                i++;
            }
            session.setAttribute("QUESTIONS",question);
            session.setAttribute("OP1",option1);
            session.setAttribute("OP2",option2);
            session.setAttribute("OP3",option3);
            session.setAttribute("OP4",option4);
    ...
    <center>
        ...
        <select name="s1">
            <%
                    ...
                    while(rs.next())
                    {
                        out.write("<option value='"+rs.getString(1)+"'>"+rs.getString("sname")+"</option>");
                    }
            %>
        </select><br><br>
        <input type="submit" name="b1" value="Continue"></form>
    </center>
     <hr>
     <div>
